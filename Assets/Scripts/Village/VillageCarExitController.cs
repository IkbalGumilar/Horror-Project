...
public sealed class VillageCarExitController : MonoBehaviour
{
    [Header("References")]
    ...
    private IEnumerator AnimatePlayerExit()
    {
        ...
        while (elapsed < duration)
        {
            ...
            Vector3 position = QuadraticBezier(startPosition, middlePosition, endPosition, smoothT);
            position.y += Mathf.Sin(smoothT * Mathf.PI) * exitStepHeight;

            playerRoot.SetPositionAndRotation(
                position,
                Quaternion.Slerp(startRotation, endRotation, smoothT));
